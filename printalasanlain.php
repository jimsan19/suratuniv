...
$con = mysqli_connect('localhost','root','','suratuniv');
   ...
   if(isset($_GET['nocut'])){
    $nocut    = $_GET['nocut'];
     ...
     $query = mysqli_query($con,"SELECT * FROM  cutialasan INNER JOIN dosen ON cutialasan.nip = dosen.nip  INNER JOIN fakultas ON cutialasan.idfak = fakultas.idfak AND nocut=".$nocut);
         $no = 1;
